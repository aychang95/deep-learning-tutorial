...
w_1 = np.random.uniform(-1*(data_dim**(-.5)), (data_dim**(-.5)), (data_dim, H_1))  # First layer in the shape of a 1000 X 2000 tensor
w_2 = np.random.uniform(-1*(H_1**(-.5)), (H_1**(-.5)), (H_1, H_2))  # Second layer in the shape of a 2000 X 100 tensor
w_3 = np.random.uniform(-1*(H_2**(-.5)), (H_2**(-.5)), (H_2, label_dim))  # Third layer in the shape of a 100 X 10 tensor
...
for i in range(1000):
    ...
    y_pred_gradient = 2 * (y_pred - y)  # Find derivative of loss in respect to y_pred: dloss/dy_pred with shape 128 X 10
